...
public class Querries {
	public static void main(String... args) {
		...
		Query NewPackage = session.createQuery("select us.us_id, us.us_name, us.us_type, ac.ac_id, ac.ac_Product_Type, ac.ac_Date_Created "
				+ " from Users as us inner join us.acount ac "
				+ "where us.us_id = :UserId");
		List<Object[]> pkg = new ArrayList<Object[]>();
		for(Long p : PackageUserId) {
			NewPackage.setParameter("UserId", p);
			pkg.addAll(NewPackage.list());
		}
		
		
		System.out.println("------------------------------------------------------------");
		for(Object[] pk : pkg) {
			System.out.println(pk[0]+"|"+pk[1]+"|"+pk[2]+"|"+pk[3]+"|"+pk[4]+"|"+pk[5]);
		}
		System.out.println("------------------------------------------------------------");
